...
#include "esphome/core/hal.h"      // For PI and potentially GPIOPin
// #include "esphome/core/defines.h" // PI is also in hal.h

#include <string> // For std::string
#include <cmath>  // For fabsf, fmodf, std::abs, cosf, sinf, sqrtf, tanf

namespace esphome {
namespace mt6701 {

// --- MT6701 Chip-Specific Constants ---
// Using static constexpr for compile-time constants.
static constexpr uint16_t MT6701_RESOLUTION_COUNTS = 1 << 14; // 14-bit resolution (16384 counts)
...
static constexpr uint8_t SSI_CRC6_ITU_POLY_REDUCED = 0x03; // Polynomial G(x)=x^6+x^1. For CRC implementations where x^n is implicit.
...
// Enum for interface type selection
enum class MT6701InterfaceType {
  NONE = 0, ///< No interface selected
  I2C,      ///< I2C interface
  SSI       ///< SSI (SPI) interface
};

// Enum for velocity filter type selection
enum class VelocityFilterType {
  NONE = 0,                 ///< No filter
  EMA,                      ///< Exponential Moving Average (1st Order IIR)
  BUTTERWORTH_2ND_ORDER     ///< Butterworth (2nd Order IIR)
};

// Enum for MT6701 I2C Registers
enum class MT6701Register : uint8_t {
  ANGLE_DATA_H = 0x03, ///< Stores Angle[13:6]
  ANGLE_DATA_L = 0x04, ///< Stores Angle[5:0] (in bits 7:2 of this register)
  // Other registers can be added here if needed (e.g., for configuration)
};

// Note on SPI Mode for MT6701:
// The MT6701 typically samples data on the RISING edge of SCLK. SCLK idles HIGH.
// This corresponds to:
//   - CPOL = 1 (Clock Polarity High, CLOCK_POLARITY_HIGH in ESPHome)
//   - CPHA = 1 (Clock Phase, sample on the second edge transition, which is RISING for CPOL=1)
//     This is CLOCK_PHASE_TRAILING_EDGE in ESPHome for CPOL=1.
// The current setting `spi::CLOCK_PHASE_LEADING_EDGE` with `spi::CLOCK_POLARITY_HIGH`
// would sample on the FALLING edge for CPOL=1.
// If the current setting works, it might be due to specific device variant or interpretation.
// Recommended to verify: spi::CLOCK_POLARITY_HIGH, spi::CLOCK_PHASE_TRAILING_EDGE
class MT6701SensorComponent : public PollingComponent, public Component, public i2c::I2CDevice,
                              public spi::SPIDevice<spi::BIT_ORDER_MSB_FIRST, 
                                                    spi::CLOCK_POLARITY_HIGH,      // CPOL = 1 (SCLK idle high)
                                                    spi::CLOCK_PHASE_LEADING_EDGE> { // CPHA = 1 (Sample on leading edge of active clock cycle)
                                                                                   // For CPOL=1, "leading edge" is the first (falling) edge.
                                                                                   // If sampling should be on RISING edge, TRAILING_EDGE is needed.
                                                                                   // Please verify with the MT6701 datasheet and testing.
 public:
  ...
  // Configuration setters (typically called from YAML via codegen)
  ...
  // void set_ssi_clock_speed(uint32_t clock_speed) { this->ssi_configured_clock_speed_ = clock_speed; } // Uncomment if used
 ...
 protected:
  ...
  // CRC methods
  uint8_t calculate_ssi_crc6_itu_(uint32_t data_18bit) const; // const as it doesn't modify object state
  ...
  // uint32_t ssi_configured_clock_speed_{4000000}; // Uncomment and integrate if custom SPI clock speed is needed
  ...
  VelocityFilterType velocity_filter_type_{VelocityFilterType::EMA}; // Default to EMA as in .cpp
  float velocity_filter_cutoff_hz_{0.0f}; 
  uint32_t min_velocity_update_period_us_{1000}; // Default value, e.g., 1ms
  ...
  uint32_t last_dt_us_for_velocity_{0}; // Stores last delta_t for EMA filter if current dt_us is too small
  ...
  float a1_{0.0f}, a2_{0.0f};           // Denominator coeffs (for 1 + a1*z^-1 + a2*z^-2), initialized for pass-through
